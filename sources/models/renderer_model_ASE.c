...
static void loadASE_parseTokens(char **tokens, int numTokens, eboolean collidable)
{
	...
	{
		// char *name, vec3_t ambient, vec3_t diffuse, vec3_t specular,
		// float shine, float shineStrength, float transparency
		model->materials.list[i].globalID = renderer_img_createMaterial(
				model->materials.list[i].diffuseMap.bitmap,
				model->materials.list[i].ambient,
				model->materials.list[i].diffuse,
				model->materials.list[i].specular,
				model->materials.list[i].shine,
				model->materials.list[i].shineStrength,
				model->materials.list[i].transparency);
	}
